...
import os
import sys
sys.path.append("/data/bhavana/wav2vec_setup/fairseq")
import editdistance
import numpy as np
import torch
...
def post_process_custom(hyp_pieces, model_path):
    # Load the SentencePiece model
    #hyp_pieces --> tok1 tok2 | tok3 tok4 |
        sp = spm.SentencePieceProcessor(model_file="/data/bhavana/Interspeech/data_fa_cp2_sp/spm_models/fa_cp2/spm_fa_cp2.model")
        tokenized_words = hyp_pieces.strip().split(" | ")
        words =[]
        for word_token in tokenized_words:
            #word_token --> tok1 tok2
            tokens = word_token.split()
            word = sp.decode(tokens)
            words.append(word)
        sentence = " ".join(words)
        return sentence
...
def process_predictions(
    ...
    for hypo in hypos[: min(len(hypos), args.nbest)]:
        # print(hypo["tokens"].int().cpu())
        # break
        hyp_pieces = tgt_dict.string(hypo["tokens"].int().cpu()) #Space separated tokens
        # print(hyp_pieces)
        if "words" in hypo:
            hyp_words = " ".join(hypo["words"])
        else:
            hyp_words = post_process_custom(hyp_pieces, args.post_process)
        # print("============")
        # print(hyp_words)
        ...
        tgt_words = post_process_custom(tgt_pieces, args.post_process)
